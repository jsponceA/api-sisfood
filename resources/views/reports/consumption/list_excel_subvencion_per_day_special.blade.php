...
<table>
    ...
    @php
        ...
        $totalColumns = 5 + ($numDays * 4) + 8 + 1; // 5 fijas + días*4 + 8 totales + 1 snacks
    ...
    @foreach ($workers as $key => $w)
        ...
        <tr>
            ...
            @foreach($periodo as $p)
                @php
                    ...
                    foreach($w->sales as $sale) {
                        ...
                        // Verificar si esta venta tiene productos de almuerzo, cena o snacks
                        $hasAlmuerzo = false;
                        $hasCena = false;
                        $hasSnacks = false;
                        ...
                        foreach($sale->saleDetails as $detail) {
                            ...
                            if($categoryName != 'ALMUERZO' && $categoryName != 'CENA') {
                                $hasSnacks = true;
                            }
                        }
                        ...
                        if($hasAlmuerzo) {
                            if($sale->deal_in_form == 'SUBVENCION') {
                                ...
                                $totalSubvencionAlmuerzo += ($sale->total_pay_company ?? 0) + ($sale->total_dsct_form ?? 0);
                            }
                        ...
                        if($hasCena) {
                            if($sale->deal_in_form == 'SUBVENCION') {
                                ...
                                $totalSubvencionCena += ($sale->total_pay_company ?? 0) + ($sale->total_dsct_form ?? 0);
                            }
                            if($sale->deal_in_form == 'SUBVENCION' && $sale->total_dsct_form > 0) {
                                //$cenaDescuento = 1;
            ...
            <!-- TOTAL DE SNACKS (productos distintos a ALMUERZO y CENA) -->
            <td style="text-align: center;border: 1px solid black;">{{number_format($totalSnacks, 2)}}</td>
        </tr>
    @endforeach
    </tbody>
</table>
