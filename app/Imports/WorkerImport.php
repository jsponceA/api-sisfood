    ...
    public function collection(Collection $collection)
    {
        $collection->each(function ( $row) {
            ...
            if (!empty($item->no_pers)){
                ...
                $verficarCargo = Charge::query()->where("name", $item->posicion_cargo)->exists();
                if (!$verficarCargo) {
                    Charge::query()->create(["name" => $item->posicion_cargo]);
                }

                $verficarCentroCosto = CostCenter::query()->where("name", $item->centro_de_coste)->exists();
                if (!$verficarCentroCosto) {
                    CostCenter::query()->create(["code" => $item->cecoste,"name" => $item->centro_de_coste]);
                }
                ...
                /*Worker::query()->create([
                    ...
                    "charge_id" => Charge::query()->where("name",$item->posicion_cargo)->first()->id,
                    ...
                    "names" => $item->nombre_de_pila,
                    "surnames" => $item->apellido." ".$item->apellido_materno,
                    "email" => null,
                    "numdoc" => $item->numero_id,
                    "phone" => null,
                    "address" => null,
